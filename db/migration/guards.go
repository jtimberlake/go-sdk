...
func IfExists(statement string) GuardFunc {
	return Guard("if exists run", func(ctx context.Context, c *db.Connection, tx *sql.Tx) (bool, error) {
		return Exists(ctx, c, tx, statement)
	})
}
...
func IfNotExists(statement string) GuardFunc {
	return Guard("if not exists run", func(ctx context.Context, c *db.Connection, tx *sql.Tx) (bool, error) {
		return NotExists(ctx, c, tx, statement)
	})
}
...
func Exists(ctx context.Context, c *db.Connection, tx *sql.Tx, selectStatement string) (bool, error) {
	if !stringutil.HasPrefixCaseless(selectStatement, "select") {
		return false, fmt.Errorf("statement must be a `SELECT`")
	}
	return c.Invoke(db.OptContext(ctx), db.OptTx(tx)).Query(selectStatement).Any()
}

// NotExists returns if a statement doesnt have results.
func NotExists(ctx context.Context, c *db.Connection, tx *sql.Tx, selectStatement string) (bool, error) {
	if !stringutil.HasPrefixCaseless(selectStatement, "select") {
		return false, fmt.Errorf("statement must be a `SELECT`")
	}
	return c.Invoke(db.OptContext(ctx), db.OptTx(tx)).Query(selectStatement).None()
}
