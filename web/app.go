...
func (a *App) logRequest(r *Ctx) {
	requestEvent := webutil.NewHTTPRequestEvent(r.Request,
		webutil.OptHTTPRequestStatusCode(r.Response.StatusCode()),
		webutil.OptHTTPRequestContentLength(r.Response.ContentLength()),
		webutil.OptHTTPRequestHeader(r.Response.Header()),
		webutil.OptHTTPRequestElapsed(r.Elapsed()),
	)
	if r.Route != nil {
		requestEvent.Route = r.Route.String()
	}
	if headers := r.Response.Header(); headers != nil {
		requestEvent.ContentType = headers.Get(webutil.HeaderContentType)
		requestEvent.ContentEncoding = headers.Get(webutil.HeaderContentEncoding)
	}
